...
def simular_cartera_con_inflacion(
    ...
    # Limit inputs to prevent runaway calculations
    tea = min(tea, 0.50)  # Max 50% annual return
    tasa_inflacion = min(tasa_inflacion, 0.20)  # Max 20% annual inflation
    años = min(años, 50)  # Max 50 years
    total_periodos = min(total_periodos, 600)  # Max 600 periods
    ...
    for periodo in range(1, total_periodos + 1):
        ...
        interes_real = interes_nominal - (saldo_inicial * inflacion_periodica)

        saldo_final = saldo_inicial + aporte_actual_periodo + interes_real

        aporte_acumulado += aporte_actual_periodo
        # Acumular la inflación como el efecto erosivo sobre el capital
